...
@implementation UIDevice (VisionHardware)
/*
 Platforms
...
- (BOOL)isDockableTelepresenceDevice
{
    ...
    return (isIpod || isIphone) && ![self isShortIpod] && ![self isIphoneThreeOrOlder];
}
...
- (BOOL)isTelepresenceController
{
    ...
    return [self isDockableTelepresenceDevice] || (isIpad && ![self isIpadOneOrOlder]);
}

- (BOOL)supportsAdvancedComputerVision
{
    return [self isTelepresenceController];
}
