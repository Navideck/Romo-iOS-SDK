...
@interface RMVisionBenchmarks : XCTestCase <RMVisionDelegate, RMVisionObjectTrackingModuleDelegate>
                             ...
                             andLoadModules:(NSArray *)modules
{
    ...
    XCTAssertNotNil(vision, @"Unable to load video file");
    
    for (NSString *module in modules) {
        BOOL loaded = [vision activateModuleWithName:module];
        XCTAssertTrue(loaded, @"Module not loaded: %@", module);
    }
    return vision;
}
         ...
         resultsDictionary:(NSMutableDictionary *)resultsDictionary
{
    ...
    XCTAssertNotNil(benchmarkDictionary, @"Unable to load benchmark dictionary");
    ...
    // Check the count
    XCTAssertEqual(benchmarkDictionary.count, resultsDictionary.count,
    ...
    for (NSString *key in benchmarkDictionary) {
        ...
        XCTAssertNotNil(result, @"Result for key: %@ not created during test", key);
        ...
        XCTAssertTrue([resultObject isApproximatelyEqual:benchmarkObject
...
- (void)testFaceDetection
{
    ...
    XCTAssertTrue([self.resultsDictionary writeToFile:filePath atomically:YES],
...
//    NSString *fileName = [NSString stringWithFormat:@"%@_results.plist", deviceSpecificPath];
//    NSString *filePath = [basePath stringByAppendingPathComponent:fileName];
//    XCTAssertTrue([self.resultsDictionary writeToFile:filePath atomically:YES], @"Failed to write resultsDictionary");
//}
