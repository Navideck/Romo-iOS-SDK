...
@interface LineTrainingBenchmarks : XCTestCase <RMVisionDelegate, RMVisionNaiveLineTrainingModuleDelegate>
...
- (void)testNaiveLineTraining
{
    ...
    XCTAssertNotNil(trainingUIImage, @"trainingUIImage is nil");
    ...
    XCTAssertNotNil(groundTruthUIImage, @"groundTruthUIImage is nil");
    ...
    XCTAssertTrue(precision >= kMinimumPrecision, @"Precision is below threshold");
    XCTAssertTrue(recall >= kMinimumRecall, @"Recall is below threshold");
    ...
    XCTAssertTrue(matIsEqual(confusionMatrix, expectedConfusionMatrix), @"Confusion matrix NOT equal to the expected confusion matrix");
...
-(cv::Mat)compareResultImage:(cv::Mat)resultImage toGroundTruthImage:(cv::Mat)groundTruthImage
{
    ...
    XCTAssertEqual(totalClassifiedPixels, totalPixelsInImage, @"Total pixels classified NOT equal to total pixels in the image");
