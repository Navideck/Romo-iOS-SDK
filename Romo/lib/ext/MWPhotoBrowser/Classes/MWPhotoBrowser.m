...
- (void)viewDidLoad {
    ...
    NSDictionary *attributes=[NSDictionary dictionaryWithObjectsAndKeys:[UIColor romoWhite],UITextAttributeTextColor, nil];
	...
	_pagingScrollView.backgroundColor = [UIColor romoBlack];
    ...
    if ([MFMailComposeViewController canSendMail] || [TWTweetComposeViewController canSendTweet] || [SLComposeViewController isAvailableForServiceType:SLServiceTypeFacebook]) {
...
- (void)actionButtonPressed:(id)sender {
    if (_actionsSheet) {
        ...
        if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
            ...
            [_actionsSheet setDelegate:self];
            
            // Add buttons from the top down
            if (NSClassFromString(@"SLComposeViewController"))
            {
                // iOS 6 -- supports facebook integration
                if ([SLComposeViewController isAvailableForServiceType:SLServiceTypeFacebook])
                    [_actionsSheet addButtonWithTitle:NSLocalizedString(@"Share on Facebook", nil)];
                if ([SLComposeViewController isAvailableForServiceType:SLServiceTypeTwitter])
                    [_actionsSheet addButtonWithTitle:NSLocalizedString(@"Share on Twitter", nil)];
            }
            else if ([TWTweetComposeViewController canSendTweet])
                [_actionsSheet addButtonWithTitle:NSLocalizedString(@"Share on Twitter", nil)];
            
            if ([MFMailComposeViewController canSendMail])
                [_actionsSheet addButtonWithTitle:NSLocalizedString(@"Email", nil)];
            [_actionsSheet addButtonWithTitle:NSLocalizedString(@"Cancel", nil)];
...
- (void)actionSheet:(UIActionSheet *)actionSheet didDismissWithButtonIndex:(NSInteger)buttonIndex {
    if (actionSheet == _actionsSheet) {
        ...
        if (buttonIndex != actionSheet.cancelButtonIndex) {
            ...
            {
                // iOS6
                if ([[actionSheet buttonTitleAtIndex:buttonIndex] isEqualToString:NSLocalizedString(@"Share on Facebook", nil)])
                {
                    [self postToSocialForServiceType:SLServiceTypeFacebook];
                    return;
                }
                else if ([[actionSheet buttonTitleAtIndex:buttonIndex] isEqualToString:NSLocalizedString(@"Share on Twitter", nil)])
                {
                    [self postToSocialForServiceType:SLServiceTypeTwitter];
                    return;
                }
            }
            else if ([[actionSheet buttonTitleAtIndex:buttonIndex] isEqualToString:NSLocalizedString(@"Share on Twitter", nil)])
...
- (void)postToSocialForServiceType:(NSString *)serviceType
{
    id <MWPhoto> photo = [self photoAtIndex:_currentPageIndex];
    if ([photo underlyingImage]) {
        ...
        [self performSelector:@selector(actuallyPostToSocialForServiceType:withPhoto:) withObject:serviceType withObject:photo];
    }
}

- (void)actuallyPostToSocialForServiceType:(NSString *)serviceType withPhoto:(id<MWPhoto>)photo
{
    if (NSClassFromString(@"SLComposeViewController"))
    {
        ...
        composeVC.completionHandler = ^(SLComposeViewControllerResult result){
            ...
            //Analytics
            [self logSharingCompletionWithServiceType:serviceType andSuccess:success];
        };
...
- (void)postToTwitter
{
    ...
    if ([photo underlyingImage]) {
        [self showProgressHUDWithMessage:[NSString stringWithFormat:@"%@\u2026" , NSLocalizedString(@"Preparing", @"Displayed with ellipsis as 'Preparing...' when an item is in the process of being prepared")]];
        [self performSelector:@selector(actuallyPostToTwitter:) withObject:photo afterDelay:0];

    }
}

- (void)actuallyPostToTwitter:(id<MWPhoto>)photo
{
    ...
    tweetSheet.completionHandler = ^(TWTweetComposeViewControllerResult result){
        ...
        //Analytics
        [self logSharingCompletionWithServiceType:@"Twitter" andSuccess:success];
        
    };
    
    UIImage *tweetImage = [self imageForPhoto:photo];
...
- (void)actuallySavePhoto:(id<MWPhoto>)photo {
    if ([photo underlyingImage]) {
        [UIImage writeToSavedPhotoAlbumWithImage:[photo underlyingImage]
...
- (void)actuallyEmailPhoto:(id<MWPhoto>)photo {
    if ([photo underlyingImage]) {
        ...
        [emailer setSubject:NSLocalizedString(@"You've got a new photo!", nil)];
        [emailer addAttachmentData:UIImagePNGRepresentation([photo underlyingImage]) mimeType:@"png" fileName:@"romo-photo.png"];
        
        NSDictionary *attributes=[NSDictionary dictionaryWithObjectsAndKeys:[UIColor romoWhite],UITextAttributeTextColor, nil];
        
        emailer.navigationBar.titleTextAttributes = attributes;
...
- (void)mailComposeController:(MFMailComposeViewController*)controller didFinishWithResult:(MFMailComposeResult)result error:(NSError*)error {
    ...
    //Analytics
    [self logSharingCompletionWithServiceType:@"Email" andSuccess:success];
    
	[self dismissModalViewControllerAnimated:YES];
}

#pragma mark - Log Completion Analytic Events --

- (void)logSharingCompletionWithServiceType:(NSString *)serviceType andSuccess:(BOOL)success
{
