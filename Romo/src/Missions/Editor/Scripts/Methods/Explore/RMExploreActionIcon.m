...
@implementation RMExploreActionIcon
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.robot.center = self.contentView.boundsCenter;
...
- (void)step
{
    ...
    static const float scurryVerticalAmount = 7.0;
    ...
    if (self.previousStepTime) {
        if (self.isStopping) {
            static const float timeWarp = 0.86;
            time = timeWarp * time;
...
{
    if (!_animationTimer) {
        __weak RMExploreActionIcon *weakSelf = self;
