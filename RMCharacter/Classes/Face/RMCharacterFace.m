...
- (void)viewDidLoad
{
    ...
    self.animation.image = [UIImage cacheableImageNamed:[NSString stringWithFormat:@"Romo_Emotion_%d.png",RMCharacterEmotionHappy]];
...
- (void)setEmotion:(RMCharacterEmotion)emotion
{
    if (!self.emoting && !self.expressing && (emotion != _emotion) && self.view.superview) {
        ...
        self.emoting = YES;
        self.animation.image = [UIImage cacheableImageNamed:[NSString stringWithFormat:@"Romo_Emotion_%d.png",emotion]];
        [_faceView addSubview:self.animation];
