...
@interface RMVisionTests : XCTestCase <RMVisionDelegate>
...
- (void)testStartingAndStoppingRapidly
{
    ...
    for (int i = 0; i < numOfSwitches; i++) {
        [self stopVisionWithPreBlock:nil andCompletion:^{
            XCTAssertTrue(!self.vision.running, @"RMVision is still initialized after being stopped");
        }];
        WaitWhile(self.isVisionRunning);
        [self setVisionToNil];
        ...
        [self startVisionWithPreBlock:nil andCompletion:^{
            XCTAssertTrue(self.vision.running, @"RMVision is not running after being started");
        }];
        WaitWhile(!self.isVisionRunning);
    }
}
...
- (void)testStartingWhileStopping
{
    ...
    XCTAssertTrue(self.vision.running, @"RMVision should be running, but is not");
...
- (void)testStoppingWhileStarting
{
    ...
    [self startVisionWithPreBlock:^{
        [self stopVisionWithPreBlock:^{
            ...
            XCTAssertFalse(self.vision.running, @"RMVision shouldn't be running, but it is");
            stopDone = YES;
...
- (void)testStartingWhileRunning
{
    ...
    [self.vision startCaptureWithCompletion:^(BOOL didSuccessfullyStart) {
        XCTAssertTrue(self.vision.running, @"RMVision is not running after being started");
        XCTAssertTrue(!didSuccessfullyStart, @"RMVision thinks it successfully started, but was already running");
        done = YES;
    }];
...
{
    [self stopVisionWithPreBlock:nil andCompletion:^{
        XCTAssertTrue(!self.vision.running, @"RMVision is running after being stopped");
    }];
    WaitWhile(self.isVisionRunning);
    
    [self stopVisionWithPreBlock:nil andCompletion:^{
        XCTAssertTrue(!self.vision.running, @"RMVision is running after calling stop");
    }];
    WaitWhile(self.isVisionRunning);
    [self setVisionToNil];
}
...
- (void)testActivatingModule
{
    ...
    XCTAssertTrue([self.vision.activeModules containsObject:self.brightnessMeteringModule], @"%@", self.vision.activeModules);
}
...
- (void)testActivatingModuleWithName
{
    ...
    XCTAssertTrue([[self.vision.activeModules valueForKeyPath:@"name"] containsObject:RMVisionModule_FaceDetection], @"%@", self.vision.activeModules);
}
...
- (void)testSettingExposurePointOfInterest
{
    if (self.vision.device.isExposurePointOfInterestSupported) {
        ...
        XCTAssertTrue([RMImageUtils isCGPoint:self.vision.exposurePointOfInterest approximatelyEqualToCGPoint:point withTolerance:FLT_EPSILON],
...
- (void)testSettingFocusPointOfInterest
{
    if (self.vision.device.isFocusPointOfInterestSupported) {
        ...
        XCTAssertTrue([RMImageUtils isCGPoint:self.vision.focusPointOfInterest approximatelyEqualToCGPoint:point withTolerance:FLT_EPSILON],
...
- (void)testDefaultExposurePointOfInterest
{
    ...
    XCTAssertTrue(CGPointEqualToPoint(self.vision.exposurePointOfInterest, defaultPoint), @"RMVision exposurePointOfInterest is %@, should be %@",
...
- (void)testDefaultFocusPointOfInterest
{
    ...
    CGPoint defaultPoint = CGPointMake(0.0, 0.0);
    XCTAssertTrue(CGPointEqualToPoint(self.vision.focusPointOfInterest, defaultPoint), @"RMVision focusPointOfInterest is %@, should be %@",
...
- (void)testFocusPointNotSavedAcrossVisionSessions
{
    if (self.vision.device.isFocusPointOfInterestSupported) {
        ...
        XCTAssertTrue(CGPointEqualToPoint(self.vision.focusPointOfInterest, point), @"RMVision focusPointOfInterest is %@, should be %@",
                     NSStringFromCGPoint(self.vision.focusPointOfInterest), NSStringFromCGPoint(point));
        
        // Stop vision
        [self stopVisionWithPreBlock:nil andCompletion:^{
            XCTAssertFalse(self.vision.running, @"RMVision is still initialized after being stopped");
        }];
        WaitWhile(self.isVisionRunning);
        [self setVisionToNil];
...
- (void)testExposurePointNotSavedAcrossVisionSessions
{
    if (self.vision.device.isExposurePointOfInterestSupported) {
        ...
        XCTAssertTrue(CGPointEqualToPoint(self.vision.exposurePointOfInterest, point), @"RMVision exposurePointOfInterest is %@, should be %@",
                     NSStringFromCGPoint(self.vision.exposurePointOfInterest), NSStringFromCGPoint(point));
...
- (void)testRunningMultipleVideoModules
{
    ...
    XCTAssertTrue([self.vision.activeModules containsObject:videoModule1], @"%@ is missing videoModule1", self.vision.activeModules);
    XCTAssertTrue([self.vision.activeModules containsObject:videoModule2], @"%@ is missing videoModule2", self.vision.activeModules);
    ...
    XCTAssertFalse([self.vision.activeModules containsObject:videoModule1], @"%@ is has videoModule1", self.vision.activeModules);
    XCTAssertFalse([self.vision.activeModules containsObject:videoModule2], @"%@ is has videoModule2", self.vision.activeModules);
...
- (void)testSettingFramerateRapidly
{
    ...
    for (int i = 0; i < NUM_FRAMERATE_CHANGES; i++) {
        ...
        XCTAssertTrue(self.vision.targetFrameRate == desiredFrameRate, @"RMVision framerate is %d, should be %d", self.vision.targetFrameRate, desiredFrameRate);
    }
}
