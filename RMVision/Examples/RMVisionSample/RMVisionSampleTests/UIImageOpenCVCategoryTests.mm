...
@interface UIImageOpenCVCategoryTests : XCTestCase
...
- (void)testGrayImage
{
    ...
    XCTAssertTrue(matEqualToSelf, @"inputImage is not equal to itself");

    UIImage *tmpUIImage = [UIImage imageWithCVMat:inputImage];
    XCTAssertNotNil(tmpUIImage, @"nil result for conversion to UIImage");
    ...
    XCTAssertTrue(isEqual, @"Nil result for conversion to UIImage");
}
...
- (void)testGrayImageROI
{
    ...
    XCTAssertTrue(roiImage.isSubmatrix(), @"roiImage should be a submatrix");

    bool matEqualToSelf = matIsEqual(roiImage, roiImage);
    XCTAssertTrue(matEqualToSelf, @"roiImage is not equal to itself");
    ...
    XCTAssertTrue(isEqual, @"Input and output images are not equal");
}
